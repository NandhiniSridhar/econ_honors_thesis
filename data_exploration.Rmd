...
```{r}
setwd("/Users/NandhiniSridhar/Desktop/honors_thesis")
...
mfdata %>% select(MFI_Name, Fiscal_Year, Period_Type, country) %>% filter(Period_Type == "ANN") %>% arrange(MFI_Name, Fiscal_Year)
mfi_names <- unique(mfdata$MFI_Name)
for(i in mfi_names){
  temp <- mfdata %>% filter(MFI_Name == i)
  cat()
}
