...
Panel of data for randomly selected MFIs
```{r}
library(dplyr)
set.seed(0)

sample <- working_mf_interest$Country.x %>% sample(5)

for(i in sample){
  cur_country <- year_totals %>% filter(Country.x == i)
...
summary(fe_inst_2_rat_reg)
```

```{r}
tbl_regression(fe_inst_2_mfi) %>% as_gt()
tbl_regression(fe_inst_2_country) %>% as_gt()
tbl_regression(fe_inst_2_reg) %>% as_gt()

tbl_regression(fe_inst_2_rat_mfi) %>% as_gt()
tbl_regression(fe_inst_2_rat_country) %>% as_gt()
tbl_regression(fe_inst_2_rat_reg) %>% as_gt()
```
