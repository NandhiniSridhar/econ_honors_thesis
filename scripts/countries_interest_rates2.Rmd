...
ggplot(year_totals, aes(x=Fiscal_Year, y=Average_Percent_Female_Borrowers)) +
  ...
  scale_x_continuous(breaks = seq(min_year_all, max_year_all, by=1)) +
  scale_y_continuous(breaks = seq(0, 1, by=0.1)) + 
  labs(title = "Average Percentage of Women Over Time Across MFIs - Working Data", x="Fiscal Year", y="Average Fraction of GLP to Female Borrowers")
...
ggplot(no_india, aes(x=Gross_Loan_Portfolio)) + 
  geom_density()
  labs(title="Density plot of GLP size for non-Indian MFIs")
...
rat_cur <- tbl_stack(
  ...
  gt::tab_header(
    title=gt::md("**Coefficients on Current Interest Rate**"),
    subtitle=gt::md("First Step of IV Regression of Fraction of GLP to Female Borrowers")
  ) %>%
  gt::fmt_number(
    columns=everything(),
    decimals = 4
